...
/**
 * Expose `Plainflow` integration.
 */

var Plainflow = exports = module.exports = integration('Plainflow')
  .option('apiKey', '')
  .option('apiHost', 'pipe.plainflow.net/v1')
...
Plainflow.prototype.initialize = function() {
  var self = this;

  if (this.options.retryQueue) {
    this._lsqueue = new Queue('plainflow', queueOptions, function(item, done) {
...
Plainflow.prototype.loaded = function() {
  return true;
};
...
Plainflow.prototype.onpage = function(page) {
...
Plainflow.prototype.onidentify = function(identify) {
...
Plainflow.prototype.ongroup = function(group) {
...
Plainflow.prototype.ontrack = function(track) {
  var json = track.json();
...
Plainflow.prototype.onalias = function(alias) {
  var json = alias.json();
...
Plainflow.prototype.normalize = function(msg) {
  ...
  if (!ctx.library) ctx.library = { name: 'pfanalytics.js', version: this.analytics.VERSION };
  var crossDomainId = this.cookie('pfl_xid');
  if (crossDomainId) {
    if (!ctx.traits) {
  ...
  msg.messageId = 'pfl-' + md5(json.stringify(msg) + uuid());
...
Plainflow.prototype.ampId = function(ctx) {
  var ampId = this.cookie('plainflow_amp_id');
...
Plainflow.prototype.enqueue = function(path, msg, fn) {
...
Plainflow.prototype.cookie = function(name, val) {
  var store = Plainflow.storage();
...
Plainflow.prototype.referrerId = function(query, ctx) {
  var stored = this.cookie('pf:context.referrer');
  var ad;
  ...
  this.cookie('pf:context.referrer', json.stringify(ad));
};
 ...
 */
Plainflow.prototype.retrieveCrossDomainId = function(callback) {
  ...
  if (!this.cookie('pfl_xid')) {
    ...
    getCrossDomainIdFromServerList(domains, writeKey, function(err, res) {
      ...
      self.cookie('pfl_xid', crossDomainId);
      // Not actively used. Saving for future conflict resolution purposes
      self.cookie('pfl_xid_fd', fromDomain);
      self.cookie('pfl_xid_ts', currentTimeMillis);
